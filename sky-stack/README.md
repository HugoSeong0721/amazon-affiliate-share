...
## 광고 — 죽은 순간이 아니라, 다음 판 앞에

탑이 무너지면 **결과 카드가 먼저** 뜬다. 점수를 보고, 자랑하고(📤), 순위를 확인하는(🏆)
동안에는 광고가 없다. 광고는 플레이어가 "다시"를 눌러 다음 판으로 넘어가는 **전환**에만 낀다
(`game.js`의 `onBeforeNextRun`). 죽자마자 광고가 덮으면 김이 새기 때문이다.

빈도도 넉넉히 띄웠다:
- 처음 6판은 무조건 무광고
- 그 뒤로는 6판마다 + 최소 4분 간격
- 8초보다 짧았던 판 뒤에는 건너뛴다 (연속 즉사에 광고까지 겹치면 최악)
- 판 도중에는 어떤 경우에도 안 뜬다

닫기 버튼은 5초 뒤에 열린다. 실제 SDK를 붙일 때는 [ads.js](./ads.js)의 provider 객체 하나만
갈아끼우면 된다. `?noads`로 끌 수 있다.
