...
// 조연들은 채도를 낮춘 한 가족의 색이다. 판에서 유일하게 쨍한 색이 택시여야
// "누가 주인공인지"를 설명 없이 말할 수 있다 — 대비는 더해서가 아니라 빼서 만든다.
export const CAR_HEX = [
  '#5d78a8', '#8a93ad', '#6f9a8d', '#9a7f9d', '#a8926b',
  '#557c9e', '#7d8daf', '#87a08a', '#a08585', '#6d86a0',
  '#94889f', '#7e97a8',
];
// 색종이만은 축제답게 쨍한 색을 쓴다
const CONFETTI_HEX = ['#ffce3a', '#4a90ff', '#ff4d5a', '#35d07f', '#a96bff', '#ff6bd0', '#2fd3d3'];
...
export class Renderer {
  ...
  _drawIntroLane(now) {
    ...
    ctx.lineDashOffset = -((now / 30) % (cell * 0.44)); // 출구 쪽으로 흐른다
    ctx.beginPath();
  ...
  _drawLot(now) {
    ...
    // 벽 대신 산울타리 — 동글동글하되 조용하게. 장식은 배경이지 주인공이 아니다.
    const mid = wall * 0.5;
    const hedge = (x0, y0, x1, y1, seed) => {
      ...
      for (let k = 0; k <= n; k++) {
        ...
        const px = x0 + (x1 - x0) * t;
        const py = y0 + (y1 - y0) * t;
        const br = wall * (0.5 + Math.sin(k * 2.1 + seed) * 0.05);
        ctx.fillStyle = k % 2 ? '#2b5a3e' : '#28553a';
        ctx.beginPath();
    ...
    // 바닥에 그려진 결승 체커선 — 골인 지점이라는 만국 공용어, 그러나 조용하게
    const sq = cell / 6;
    const fx0 = gx + wall * 0.18;
    for (let cxk = 0; cxk < 2; cxk++) {
      for (let cyk = 0; cyk < 6; cyk++) {
        ctx.fillStyle = (cxk + cyk) % 2 ? 'rgba(240,244,255,0.85)' : 'rgba(20,26,44,0.85)';
        ctx.fillRect(fx0 + cxk * sq * 0.7, gapY0 + cyk * sq, sq * 0.7, sq);
      }
    }
  ...
  _drawCarBody(i, hex, L, Wd, now, isPlayer) {
    ...
    // 앞유리 → 지붕 → 뒷유리. 조연의 유리는 몇 톤 어둡게 가라앉힌다.
    const glass = ctx.createLinearGradient(0, by + bl * 0.15, 0, by + bl * 0.36);
    glass.addColorStop(0, isPlayer ? '#dcecff' : 'rgba(200,215,238,0.5)');
    glass.addColorStop(1, isPlayer ? '#9dbde8' : 'rgba(140,162,196,0.5)');
    ctx.fillStyle = glass;
    ...
    if (v.len === 3) {
      ctx.fillStyle = isPlayer ? 'rgba(157,189,232,0.9)' : 'rgba(157,180,215,0.45)';
      for (let k = 0; k < 3; k++) {
    ...
    if (isPlayer) {
      ...
      this._drawTaxiTrim(bw, bl, by, roofTop, roofH, now);
    }
  }
