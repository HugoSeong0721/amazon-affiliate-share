...
| 5장 광야의 시험 | 학습형 ★ | 길에서 주운 말씀 카드로 유혹을 물리친다 |
...
6. 5장(광야): 사막 길 세 구간. 구간마다 구절 하나를 조각으로 줍고, 구간 끝에서 어두운 목소리가
   유혹합니다. 카드 셋 중 **방금 주운 그 구절**을 골라 만지면 바위 문이 무너지고 길이 열립니다.
   틀려도 벌칙 없음 — 한 번은 "다시 생각해볼까?", 두 번은 힌트, 세 번부터는 빛기둥이 정답 카드를
   가리킵니다. 카드에는 내가 모은 조각만 보이고 나머지는 빈칸입니다.
7. 6장(그물): 부두 끝에서 그물을 던지면 **반드시 빈 그물**이 나옵니다. 그게 이야기입니다 —
   그 후에야 깊은 부두가 생기고, 해변의 빛이 말을 겁니다.
8. 10장(오병이어): 바구니는 누군가 도시락을 내놓기 전까지 비어 있습니다. 첫 터치가 나눔,
...
(현재 60개). `tests/` 에 `*.spec.luau` 파일을 추가하면 등록 없이 자동 발견됩니다.
...
| `TrialLogic.spec` | 5장의 규칙 — 오답은 무벌칙, 시험은 순서대로, 3회 오답이면 정답을 알려주는 자비 규칙 |
| `Verses.spec` | 구절 테이블 불변식 — 조각 ID 중복·미아 금지(저장 키로 쓰임), 빈칸 렌더링, 다국어 |
