...
[DESIGN.md](DESIGN.md)의 P0~P1 구현. 허브(안개 낀 길)와 네 챕터 — MVP가 검증해야 할
세 가지 원형이 전부 들어 있습니다:

| 챕터 | 원형 | 배우는 것 |
|---|---|---|
| 0장 기다림 | 튜토리얼 | 걷기·점프·만지기 |
| 1장 베들레헴의 밤 | 탐색형 | 뛰면 흩어지고 걸으면 모인다 (목자) |
| 6장 그물을 던져라 | 순종형 | 빈 그물 다음에야 깊은 곳이 열린다 |
| 10장 오병이어 | 협동형 | 먼저 내놓아야 바구니가 차고, 함께 나누면 빨라진다 |
...
6. 6장(그물): 부두 끝에서 그물을 던지면 **반드시 빈 그물**이 나옵니다. 그게 이야기입니다 —
   그 후에야 깊은 부두가 생기고, 해변의 빛이 말을 겁니다.
7. 10장(오병이어): 바구니는 누군가 도시락을 내놓기 전까지 비어 있습니다. 첫 터치가 나눔,
   그 다음 터치부터 빵을 받아 다섯 무리에게 나릅니다. 모두 먹이면 열두 바구니가 남습니다.
   혼자서도 깰 수 있지만, 나눈 사람이 많을수록 한 번에 나르는 양이 늘어납니다.
...
(현재 44개). `tests/` 에 `*.spec.luau` 파일을 추가하면 등록 없이 자동 발견됩니다.
...
| `FeedingLogic.spec` | 10장의 교훈 그 자체 — 내놓기 전엔 빈 바구니, 내놓은 뒤엔 마르지 않고, 함께 나누면 빨라진다 |
...
| 협동 게이트 | 10장은 나눈 인원수만큼 운반량이 늘어나는 소프트 협동. 4인 하드 게이트(9·16장) 여부는 P1 테스트에서 결정 |
