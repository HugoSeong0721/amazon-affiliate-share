...
export function paramsAt(height) {
  ...
  const ramp = 1 - Math.exp(-h / 900); // 0 → 1 포화 곡선
  return {
    speed: 42 + 58 * ramp,                        // 42 → 100
    hazardChance: Math.min(0.9, 0.15 + 1.1 * ramp), // 구간당 소행성 확률
...
function release(state) {
  const o = state.orbit;
  state.dirX = -Math.sin(o.theta) * o.dir;
  state.dirY = Math.cos(o.theta) * o.dir;
  state.orbit = null;
  state.mode = 'flying';
